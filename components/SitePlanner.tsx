...
import { AppStep, SitemapPage, PageWireframe, SavedSitePlan, WireframeSection, ChecklistSection } from '../types';
import { 
  generateSitemap, 
...
export const SitePlanner: React.FC<SitePlannerProps> = ({
  ...
  const [ldJsonSchema, setLdJsonSchema] = useState<string | null>(null);
  ...
  const resetEnhancementsState = () => {
    setLdJsonSchema(null); setIsLoadingLdJson(false); setErrorLdJson(null);
    setSiteSuggestions(null); setIsLoadingSiteSuggestions(false); setErrorSiteSuggestions(null);
    setGoLiveChecklist(null); setIsLoadingGoLiveChecklist(false); setErrorGoLiveChecklist(null);
    setWebDevChecklist(null); setIsLoadingWebDevChecklist(false); setErrorWebDevChecklist(null);
  };
  ...
  }, [setSitemap, setPageWireframes, setCurrentStep, setError, setCompanyDescription]);
  ...
  useEffect(() => {
    ...
    if (savedDataJson) {
      try {
        ...
        if (parsedData.sitemap && parsedData.sitemap.length > 0) {
          const allWireframesLoaded = parsedData.pageWireframes && parsedData.pageWireframes.length === parsedData.sitemap.length &&
                                    !parsedData.pageWireframes.some(pw => !pw.sections || pw.sections.length === 0);
          if (allWireframesLoaded) {
            setCurrentStep(AppStep.WIREFRAME_DISPLAY);
          } else {
  ...
  const handleDownloadPlan = () => {
    ...
    showFeedback("Site plan downloaded!");
    setIsSettingsOpen(false);
  };
  ...
  const handleResetAllData = () => {
    if (window.confirm("Are you sure you want to reset all data? This will clear any saved plan and start over.")) {
      localStorage.removeItem(LOCAL_STORAGE_KEY);
      handleStartOver(); 
      setTemperature(0.7); 
      resetEnhancementsState();
  ...
  const handleDescriptionSubmit = async () => {
    ...
    resetEnhancementsState(); // Clear old enhancements when generating new plan
  ...
  const triggerWireframeGeneration = async (currentSitemap: SitemapPage[], currentCompanyDescription: string, currentTemperature: number) => {
     if (!API_KEY_CONFIGURED) {
        setError("API Key is not configured for wireframe generation.");
        setCurrentStep(AppStep.ERROR); 
        setIsLoading(false);
    ...
    setError(null); 
    resetEnhancementsState(); // Clear enhancements if wireframes are regenerated
    ...
    for (let i = 0; i < currentSitemap.length; i++) {
      ...
      setPageWireframes(prevPws => prevPws!.map(pw => pw.pageId === page.id ? {...pw, isLoading: true} : pw ));
      ...
      setPageWireframes(currentSitemap.map(sitemapPage => {
        ...
        const existingOrInitial = pageWireframes?.find(opw => opw.pageId === sitemapPage.id);
  ...
  const handleRegenerateSitemapOnly = () => {
     if (!companyDescription.trim()) {
      setError("Please describe your company or website idea to regenerate the sitemap.");
      return;
    }
    setSitemap(null);
    setPageWireframes(null);
  ...
  const handleRegenerateWireframesOnly = async () => {
    if (sitemap && sitemap.length > 0) {
      ...
      setError("Cannot regenerate wireframes without a sitemap. Please generate sitemap first.");
      setCurrentStep(AppStep.DESCRIPTION_INPUT);
    }
  }
  ...
  const handleGenerateLdJson = async () => {
    ...
    try {
      const schema = await generateLdJsonSchema(companyDescription, sitemap, temperature);
      setLdJsonSchema(schema);
    } catch (e) {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
      <div className="bg-white p-6 sm:p-8 md:p-12 rounded-xl shadow-2xl max-w-4xl mx-auto">
        {/* Settings Panel and Initial Input Section (no changes here beyond what's already present) */}
          ...
          <div className="relative">
            ...
            {isSettingsOpen && (
              <div 
                ...
                className="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-lg shadow-xl p-4 z-20 space-y-4"
                role="dialog"
                aria-modal="true"
              ...
              >
                ...
                <div>
                  ...
                  <p className={`text-xs p-2 rounded-md ${API_KEY_CONFIGURED ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                    {API_KEY_CONFIGURED ? "Configured" : "Not Configured"}
                  </p>
                   {!API_KEY_CONFIGURED && <p className="text-xs text-red-600 mt-1">Set <code>API_KEY</code> in environment.</p>}
                </div>
                <div>
                ...
                <div className="border-t border-slate-200 pt-3 space-y-2">
                   ...
                   <Button onClick={handleDownloadPlan} size="sm" className="w-full flex items-center justify-center" variant="outline" disabled={!sitemap}>
                     <Icon name="DownloadIcon" className="w-4 h-4 mr-2"/> Export Full Site Plan (JSON)
                   </Button>
        ...
        {!API_KEY_CONFIGURED && currentStep !== AppStep.ERROR && (
            <div className="mb-6 p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700 rounded-md" role="alert">
                ...
                <p>The AI Site Planner requires an API Key to function. Please ensure the <code>API_KEY</code> is set in your environment. Open "AI Settings" for more info.</p>
            </div>
        )}
                ...
                {currentStep === AppStep.DESCRIPTION_INPUT || (currentStep === AppStep.ERROR && !sitemap) ? (
                  ...
                  sitemap && ( 
                    <div className="flex flex-col sm:flex-row gap-2">
                       <Button onClick={handleRegenerateSitemapOnly} variant="outline" className="w-full" disabled={isLoading || !companyDescription.trim() || !API_KEY_CONFIGURED}>
                          Regenerate Full Plan (Uses AI Settings)
                       </Button>
                    </div>
                  )
                )}
                {sitemap && sitemap.length > 0 && (currentStep === AppStep.SITEMAP_DISPLAY || currentStep === AppStep.WIREFRAME_DISPLAY || currentStep === AppStep.WIREFRAME_GENERATION_LOADING ||(currentStep === AppStep.ERROR && sitemap)) && <SitemapReviewChecklist />}
            </div>
        )}

        {/* Sitemap and Wireframes Display (no changes here beyond what's already present) */}
        {sitemap && (currentStep === AppStep.SITEMAP_DISPLAY || currentStep === AppStep.WIREFRAME_GENERATION_LOADING || currentStep === AppStep.WIREFRAME_DISPLAY || (currentStep === AppStep.ERROR && sitemap)) && !isLoading && (
          <div className="mt-2 pt-8 border-t border-slate-200">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
              ...
              <Button onClick={handleStartOver} variant="outline" size="sm" className="text-red-600 border-red-600 hover:bg-red-50" disabled={isLoading}>
                  Start Over (New Idea)
              </Button>
            </div>
              ...
              {sitemap.map((page, pageIndex) => {
                ...
                return (
                  <div key={page.id} className="page-flow-item-container bg-slate-50 p-1 rounded-lg shadow-md">
                    <details className="group" open={pageIndex === 0 || !!wireframeForPage || hasWireframeError || isLoadingWireframe }>
                        ...
                        {!isLoadingWireframe && wireframeForPage && wireframeForPage.sections.length > 0 && (
                             <div className={`mt-1 p-4 ${hasWireframeError ? 'bg-red-50' : 'bg-sky-50/50'}`}>
                              ...
                              <div className="space-y-3">
                                  ...
                                  <div key={section.id} className={`p-3 rounded-md border shadow-sm ${hasWireframeError ? 'bg-red-100 border-red-300' : 'bg-slate-100 border-slate-300 hover:shadow-md transition-shadow'}`}>
                                      <p className={`text-sm font-medium ${hasWireframeError ? 'text-red-600' : 'text-slate-700'}`}>{section.sectionName}</p>
                                      <p className={`text-xs ${hasWireframeError ? 'text-red-500' : 'text-slate-500'} mt-1 italic`}>
            ...
            {!isLoading && sitemap && sitemap.length > 0 && currentStep !== AppStep.WIREFRAME_GENERATION_LOADING && (
                <div className="mt-8 pt-6 border-t border-slate-200 flex flex-col sm:flex-row gap-3">
                    ...
                    {(currentStep === AppStep.WIREFRAME_DISPLAY || (currentStep === AppStep.ERROR && pageWireframes?.some(pw => pw.sections.length > 0))) && (
                        <Button 
                            onClick={handleRegenerateWireframesOnly} 
                            variant="outline" 
                            className="w-full" 
                            disabled={isLoading || !API_KEY_CONFIGURED}
                        >
                            Regenerate Wireframes (Uses AI Settings)
                        </Button>
                    )}
                </div>
            )}
        ...
        {/* Enhancements & Checklists Section */}
        {currentStep === AppStep.WIREFRAME_DISPLAY && !isLoading && sitemap && pageWireframes && API_KEY_CONFIGURED && (
          <div className="mt-12 pt-8 border-t-2 border-indigo-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-slate-50 p-5 rounded-lg shadow">
                <div className="flex items-center mb-3">
                  ...
                  <h4 className="text-lg font-semibold text-slate-700">LD+JSON Schema</h4>
                </div>
                ...
                {!isLoadingLdJson && ldJsonSchema && (
                  <pre className="bg-slate-100 p-3 rounded-md text-xs overflow-x-auto whitespace-pre-wrap break-all">
                    <code>{ldJsonSchema}</code>
                  </pre>
                )}
                {!isLoadingLdJson && !ldJsonSchema && (
                    <p className="text-sm text-slate-500 mb-3">Generate a basic LD+JSON schema for your website to improve SEO visibility.</p>
                )}
                <Button onClick={handleGenerateLdJson} disabled={isLoadingLdJson || !API_KEY_CONFIGURED} size="sm" className="mt-3 w-full">
                  {ldJsonSchema ? "Regenerate LD+JSON" : "Generate LD+JSON"}
                </Button>
              </div>
              ...
              <div className="bg-slate-50 p-5 rounded-lg shadow">
                ...
                {!isLoadingSiteSuggestions && siteSuggestions && (
                 ...
                 {!isLoadingSiteSuggestions && !siteSuggestions && (
                    <p className="text-sm text-slate-500 mb-3">Get AI-powered suggestions to enhance your website's content, UX, or features.</p>
                )}
              ...
              <div className="bg-slate-50 p-5 rounded-lg shadow">
                ...
                {!isLoadingGoLiveChecklist && goLiveChecklist && (
                 ...
                 {!isLoadingGoLiveChecklist && !goLiveChecklist && (
                    <p className="text-sm text-slate-500 mb-3">Generate an AI-curated checklist for essential pre-launch verifications.</p>
                )}
              ...
              {/* Web Development Checklist Card */}
              <div className="bg-slate-50 p-5 rounded-lg shadow">
                <div className="flex items-center mb-3">
                ...
                {!isLoadingWebDevChecklist && !webDevChecklist && (
                    <p className="text-sm text-slate-500 mb-3">Get an AI-generated list of key tasks for the website development phase.</p>
                )}
            ...
            {/* Static SEO Checklist - Full Width */}
